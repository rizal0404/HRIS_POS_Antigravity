...
const PengajuanPage: React.FC<PengajuanPageProps> = ({ user }) => {
    ...
    return (
        <>
            <div className={`p-6 transition-all duration-300 ${isModalOpen ? 'blur-md grayscale-[20%] scale-[0.99] pointer-events-none' : ''}`}>
            ...
            {toast && (
                <div
                    className={`fixed top-4 right-4 z-[2000] px-4 py-2 rounded-md shadow-md text-sm text-white animate-fade-in ${toast.type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}
                >
                    {toast.message}
                </div>
            )}

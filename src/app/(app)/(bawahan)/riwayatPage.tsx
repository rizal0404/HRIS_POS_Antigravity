...
const getStatusBadge = (status: string) => {
    ...
    if (status === AttendanceStatus.ON_TIME || status === RequestStatus.APPROVED) {
        return <span className={`${baseStyles} bg-green-50 text-green-700 border-green-200`}>Disetujui / On Time</span>;
    }
    if (status === AttendanceStatus.LATE) {
        return <span className={`${baseStyles} bg-yellow-50 text-yellow-700 border-yellow-200`}>Terlambat</span>;
    }
    if (status === AttendanceStatus.ABSENT || status === RequestStatus.REJECTED) {
        return <span className={`${baseStyles} bg-red-50 text-red-700 border-red-200`}>Absen / Ditolak</span>;
    }
    if (status === RequestStatus.PENDING) {
        return <span className={`${baseStyles} bg-blue-50 text-blue-700 border-blue-200`}>Menunggu</span>;
    }

    // Default fallback
    return <span className={`${baseStyles} bg-gray-50 text-gray-600 border-gray-200`}>{status.replace(/_/g, ' ')}</span>;
};
...
const RiwayatPage: React.FC<RiwayatPageProps> = ({ user }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 font-sans">
            {/* Header */}
            <header className="flex-none px-6 py-5 bg-white border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-xl font-bold text-slate-900">Riwayat Aktivitas</h1>
                    <p className="text-sm text-slate-500 mt-1">Pantau kehadiran dan status pengajuan Anda.</p>
                </div>
                <button
                    onClick={handleOpenKoreksiLatest}
                    className="inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition-colors shadow-lg shadow-slate-200"
                >
                            ...
                            <div className="flex flex-1 gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        ...
                                        className="block w-full pl-10 pr-3 py-2.5 text-sm border-slate-200 focus:ring-blue-500 focus:border-blue-500 rounded-xl bg-slate-50 text-slate-700 font-semibold"
                                    />
                                </div>
                                <div className="relative flex-1">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <span className="text-xs text-slate-400 font-medium">Smp</span>
                                    </div>
                                    <input
                                        type="date"
                                        value={endDate}
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-dashed border-slate-200">
                            <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4">
                                <CalendarIcon className="w-8 h-8 text-slate-300 text-[32px]" />
                            </div>
                            <h3 className="text-lg font-bold text-slate-900">Tidak ada riwayat ditemukan</h3>
                            ...
                            {filteredHistory.map(item => {
                                ...
                                return (
                                    <Card
                                        ...
                                        className="p-4 hover:shadow-md transition-shadow cursor-pointer border border-slate-100 group"
                                        onClick={() => handleDetailClick(item)}
                                    >
                                        <div className="flex items-center gap-4">
                                            {/* Icon Box */}
                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 ${isAttendance ? 'bg-blue-50' :
                                                    item.request_type === RequestType.CUTI ? 'bg-orange-50' :
                                                        item.request_type === RequestType.SAKIT ? 'bg-red-50' : 'bg-slate-50'
                                                }`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start mb-1">
                                                    <h3 className="font-bold text-slate-900 text-sm truncate pr-2 capitalize">
